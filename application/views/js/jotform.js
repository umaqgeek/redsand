        ...
        if(type === "radio" || type === "checkbox") {
            try {
                ...
                for(var i=0; i < length-1; i++) {
                    ...
                    if($("id_"+id).select('.form-'+type+'-item')[i].next()) {
                        $("id_"+id).select('.form-'+type+'-item')[i].next().insert({after: swappedOut});
                    }
                }
...
{       // Event simulator
    ...
    */
    ...
    checkCalculation: function(calc) {
        ...
        if(parseFloat(output) === 0 && $('input_' + result) && $('input_' + result).readAttribute('defaultValue') != null) {
    ...
    */
    ...
    track: function(w, d){
        ...
        var _referer;
        try {
            _referer = encodeURIComponent(window.top.location.href);
        } catch(e) {
            _referer = 'undefined'
        }
        ...
        if(_form) {
          insertAfter(createImageEl(), _form);
          createEventID();
        }
        ...
        function createImageEl() {
          ...
          if(typeof _referer === 'string') {
            if(_referer.length > 0) {
              src = src + '?ref=' + _referer;
              if(resolutionStr){
                src = src + '&res=' + resolutionStr;
              }
            } else {
              if(resolutionStr){
                src = src + '&res=' + resolutionStr;
        ...
        function createEventID() {
          var eventID = 1*new Date()+'_'+_formID+'_'+randomString(7);
          var inputEl = document.createElement('input');
          inputEl.setAttribute('type', 'hidden');
          inputEl.setAttribute('name', 'event_id');
          inputEl.value = eventID;
